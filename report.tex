...
\textbf{Lean correspondence.}
We use a similar structure as described above. We show the first part with the lemmas \texttt{insert\_contains\_inserted} and \texttt{contains\_then\_insert\_contains} for one direction and \texttt{insert\_- contains\_then\_contains\_or\_inserted} for the other one, respectively. All are shown with functional induction over \texttt{insert}.
...
Moreover, we show that extraction preserves the membership of all elements except for the removed root. If the original tree is a node with the root value $v$, the resulting tree will have all elements except $v$. Both can be shown via functional induction on the definition and by case distinction if the element contained is the root or not.
...
\textbf{Lean correspondence.}
 The proof proceeds by a small case split on whether the boolean membership test containsb finds the element: if the element is present the implementation performs remove followed by insert, otherwise it is a no-op. The lemma \texttt{contains\_then\_decrease\_priority\_contains} shows that any element of the original tree remains present after \texttt{decrease\_priority}, and it is proved by structural induction on \texttt{decrease\_priority} (and by delegating to containment lemmas for \texttt{remove} and \texttt{insert}). The lemma \texttt{decrease\_priority\_contains\_then\_contains} proves the converse direction, that any element found after the operation already belonged to the original tree; this uses shape facts about \texttt{insert} and \texttt{remove} together with the helper \texttt{containsb\_contains} to relate the boolean test to propositional membership. The preservation of the heap property is established in \texttt{decrease\_priority\_preserves\_min\_heap}, which reasons by the two cases: the no-op case is trivial, and the present-element case composes \texttt{remove\_preserves\_min\_heap} and \texttt{insert\_preserves\_min\_heap} to show the final tree remains a min-heap. Crucial auxiliary lemmas used in these arguments include \texttt{remove\_preserves\_members\_except\_v} (so removing does not accidentally drop unrelated members), \texttt{insert\_contains} and \texttt{insert\_contains\_then\_contains\_or\_inserted} (to reason about the newly inserted element vs.\ existing ones), and the shape lemma \texttt{insert\_is\_node} that enables case analysis on constructed nodes. The proof strategy is therefore to reduce correctness of \texttt{decrease\_priority} to correctness of \texttt{remove} and \texttt{insert}, plus a small bridge from the boolean membership test to the propositional predicates. Combining these lemmas yields the final equivalence and the heap-preservation clause, completing the proof of \texttt{decrease\_priority\_correctness}.
...
\textbf{Completed work.} Our formalization of the Binary Heap is composed of the data structure toghether with its invariant to ensure that the binary tree is indeed a min heap and the operations on it. Among the results established are: 
\begin{itemize}
  ...
  \item proofs of heap invariant preservation of the operations and the auxiliary operations that werde defined in the process
  \item proof of membership preservation to prove only neccessary elements were added/removed 
  \item proofs of correctness combining the heap preservation and membership preservation properties
  \item providing a binary heap interface for Dijkstsra's shortest path algorithm including additional helper lemmas including size and stuctural invariants
\end{itemize}


\textbf{Potential further steps.} Possible next steps would be to transfer the tree-based implementation to an array-based one. This will allow for the precise implementation of the operation's algorithm and performing runtime analysis on them. As most proofs were shown by functional induction, it can be assumed that they can be reused in some extent.
\newpage
...
We prove that the heap measure strictly decreases, i.e. $\texttt{queue''.sizeOf} < \texttt{queue.sizeOf}$. Starting from a nonempty heap (the hypothesis $hq$ that $\texttt{queue} \neq \texttt{queue.empty}$ is used), the proof exposes the concrete definitions of the intermediate queues with $hq'\_eq$ and $hq''\_eq$ (both proven via reflexitivity because queue' and queue'' are exactly the second components of extract\_min and relax\_neighbors respectively). The \texttt{calc} chain then rewrites \texttt{queue''.sizeOf} to \texttt{(relax\_neighbors g u dist queue').2.sizeOf}, applies the lemma \texttt{sizeOf\_relax\_neighbors\_le} to get a non‑strict inequality to \texttt{queue'.sizeOf}, rewrites \texttt{queue'.sizeOf} to the second component of extract\_min, and finally uses an exposed lemma from the BinaryHeap interface to get the strict decrease after extraction; together these steps yield the required strict decrease.\\
...
\subsection{Challenges in Formalizing Dijkstra's Algorithm}

Proving the Dijkstra lemmas turned out to be considerably more difficult than anticipated. One major challenge was that Lean's automated tools often failed to handle the complexity of the expressions involved. For instance, tactics such as \texttt{simp}, \texttt{grind}, or even \texttt{exact?} frequently got stuck, particularly when simplifying goals that depended on the recursive definition of \texttt{dijkstra\_rec}. In many cases, the recursion depth limit was reached, preventing the automation from completing. To overcome this, we had to manually break proofs into smaller steps and carefully control the order and application of simplifications. This process required significant attention to detail and consumed substantial time.

Another substantial difficulty was determining the correct form of induction. Many proofs required strong induction on the size of the heap, which tracks the vertices yet also involves additional information, such as the current distance map and the remaining queue of vertices. This necessitated the formulation of very precise statements for each inductive step, ensuring that all relevant data was correctly handled. Developing and verifying these statements involved extensive trial and error. Despite successfully formalizing nearly all of the lemmas, one key result—the search lemma (\texttt{exists\_extract\_or\_top})—remains unproven because we were unable to properly align the induction hypothesis with the recursion.

Overall, the most challenging aspect of the formalization was converting intuitive reasoning into fully rigorous proofs. Proofs of Dijkstra's algorithm available in the literature often rely on high-level intuition, where a single sentence in a paper could correspond to hundreds of lines of formal Lean code. This required breaking each argument into very small, verifiable components and proving them individually. The entire process was slow, iterative, and demanded meticulous attention to detail.

In total, the time required for this project exceeded initial expectations, reflecting both the complexity of the algorithm and the rigor needed for a complete formalization.
...
% TODO? Make distances and priority queue a fixed pair.

\subsection{What we have learned}
A main takeaway from this project is the importance of modularity. When proving the theorems and lemmas, it was helpful to divide bigger proof goals into smaller ones for reusability as well as for minimizing the complexity. Breaking up lemmas and proving a more general statement furthermore helped to be able to perform functional induction which was sometimes not possible on the more specific statements. We always tried to make the statements as specific as needed and as general as possible. By formalizing the Binary Heap and Dijkstra's shortest path algorithms, we learned how to apply formal methods on real-world algorithms.
