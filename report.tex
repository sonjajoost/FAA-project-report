...
As mentioned, \texttt{decrease\_priority} is built on top of \texttt{insert} and \texttt{remove}, so we showed the corresponding correctness lemmas for \texttt{remove} (\texttt{as correctness of insert is already shown above}) using functional induction and could combine the lemmas with the auxiliary lemmas from \texttt{insert} to prove the final goals.
